...
from .utils import frame_audio,frame2vector,vector2frame
...
class Audio_processor:
    def __init__(self,sr,freqs,frame_ms=25.0,hop_ms=25.0):
        self.sr=sr
        self.freqs=freqs
        self.frame_ms=frame_ms
        self.hop_ms=hop_ms
        self.frame_seqlen=sr*int(frame_ms)//1000
    def frame2spec(self,frames):
        return frame2vector(frames,self.sr,self.freqs)
    def spec2frame(self,specs_real,spec_imag):
        return vector2frame(specs_real,spec_imag,self.sr,self.freqs,self.frame_seqlen)
...
class ComplexLinear(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        # ---- 定义参数（全部实数）----
        # U = U_r + i U_i
        self.U_real = nn.Parameter(torch.empty(out_features, in_features))
        self.U_imag = nn.Parameter(torch.empty(out_features, in_features))
    ...
    def forward(self, x_real: torch.Tensor, x_imag: torch.Tensor):
        x_cat = torch.cat([x_real, x_imag], dim=-1)     # (B, 2*in)

        # block 构造
        M11 = self.U_real + self.W_real
        M12 = -self.U_imag + self.W_imag
        M21 = self.U_imag + self.W_imag
        M22 = self.U_real - self.W_real

        top = torch.cat([M11, M12], dim=1)
        bot = torch.cat([M21, M22], dim=1)
        M   = torch.cat([top, bot], dim=0)      # (2*out, 2*in)

        # ===== 一次 matmul 计算 =====
        y_cat = F.linear(x_cat, M)              # (B, 2*out)

        # ===== 拆回 =====
        out_real, out_imag = torch.split(y_cat, self.out_features, dim=-1)    
...
class ComplexNetLM(PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: ComplexNetConfig,sr,freqs,frame_ms,hop_ms):
        ...
        self.frame_ms=frame_ms
        self.hop_ms=hop_ms
        self.audio_processor=Audio_processor(self.sr,self.freqs,self.frame_ms,self.hop_ms)
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            std = std / math.sqrt(2)
            torch.nn.init.normal_(module.U_real, mean=0.0, std=std)
            torch.nn.init.normal_(module.U_imag, mean=0.0, std=std)
            torch.nn.init.normal_(module.W_real, mean=0.0, std=std)
            torch.nn.init.normal_(module.W_imag, mean=0.0, std=std)
            if module.bias_real is not None:
                torch.nn.init.zeros_(module.bias_real)
                torch.nn.init.zeros_(module.bias_imag)
    ...
    def embed(
        ...
        # input_ids here are actually audio frames [Batch, Seq_Len, Frame_Len]
        # Perform FFT inside the model
        
        # Ensure audio_processor uses the correct device freqs
        self.audio_processor.freqs = self.freqs
        
        embedding = self.audio_processor.frame2spec(input_ids)
        return embedding.real, embedding.imag
    ...
    def token_logits(
        ...
        logits=self.audio_processor.spec2frame(x_real,x_imag)
        return logits
